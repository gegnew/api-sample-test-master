...
const processCompanies = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    while (tryCount <= 4) {
      try {
        searchResult =
          await hubspotClient.crm.companies.searchApi.doSearch(searchObject);
        break;
      } catch (err) {
        tryCount++;
...
const processContacts = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    while (tryCount <= 4) {
      try {
        searchResult =
          await hubspotClient.crm.contacts.searchApi.doSearch(searchObject);
        break;
      } catch (err) {
        tryCount++;
...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // could also be createdAt > updatedAt
      const isCreated =
      ...
      const meetingProperties = {
        ...
        contact_email: contactData.properties.email,
        ...meeting.properties,
      };
